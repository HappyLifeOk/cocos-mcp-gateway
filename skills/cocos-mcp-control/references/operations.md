...
1. 确认项目存在 `extensions/cc-3-8-x-mcp`。用户要求接入而扩展不存在时，运行 `cocos-mcp-connect <项目绝对路径>`；该命令不会覆盖已有扩展。
...
- 注册存在但 endpoint 不通：等待扩展完成启动；仍失败时由用户决定是否重启扩展或编辑器。
- offline 修改后界面不变：直接写盘不会自动触发 Cocos 导入，必须重导资源并按项目约定刷新预览。
- 工具前缀变化：重新调用 `router_list_editors`，以当前返回的 `shortName` 为准。
