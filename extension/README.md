...
把编辑器的 scene/asset/preview/local 能力以 MCP 协议暴露给 Codex、Claude Code 等客户端；同时提供无需编辑器运行的 offline prefab 编辑能力，支持节点增删克隆与 stub 节点属性覆写。
...
## 接入 Codex、Claude Code 和其他 MCP 客户端
...
推荐只安装一份全局 router。各 Cocos 项目只运行自己的编辑器扩展并写入
`~/.cocos-mcp/editors/*.json`；不要为每个项目重复启动 router。

新项目尚未安装扩展时，可用共享安装提供的命令接入。已有目标目录时命令只检查，
不会覆盖项目自己的扩展：
...
Codex 使用个人插件时，由插件的 `.mcp.json` 启动共享 router。Claude Code 使用同一份
runtime：

```bash
claude mcp add --scope user cocos -- \
  node /path/to/cocos-mcp/runtime/router/bin.js
```

其他支持 stdio MCP 的客户端使用等价配置：

```json
{
  "command": "node",
  "args": ["/path/to/cocos-mcp/runtime/router/bin.js"]
}
```

客户端接入后即可调用所有活跃编辑器的 tool，以及全局 offline prefab tools。项目内旧的
同名 MCP 配置会与用户级配置冲突；切换到全局 router 后应移除项目级 `cocos` 条目。

扩展中的 router 或 CLI 更新后，用共享安装自带的同步脚本刷新 runtime：

```bash
node /path/to/cocos-mcp/scripts/sync-runtime.js \
  /path/to/cc-3-8-x-mcp
```

Claude Code 等直接指向共享目录的客户端会使用新 runtime；Codex 插件采用安装缓存，
同步后还需更新插件 cachebuster 并重新安装，再在新任务中验证。
