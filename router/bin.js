 ...
 *
 * 客户端接入：
 *   Codex 插件或任意 stdio MCP 客户端只需全局注册这一份 router。
 */
...
async function discover() {
    ...
    var seen = new Set();
    var beforeSig = toolListSignature();   // 改动前的工具签名，末尾比对决定是否通知客户端
    for (var i = 0; i < entries.length; i++) {
        ...
        // 否则客户端永远看不到后激活的工具（如 forest__preview_refresh_and_reload）。
    ...
    dedupeShortNames();
    // 工具列表变化（编辑器增减 / 某编辑器工具数从不全变全）→ 通知客户端重新拉 tools/list。
    // 否则客户端停在初次 tools/list 的旧快照：editor 慢启动时初次只暴露 router 自带工具，
    // 后激活的 forest__ 业务工具永远拿不到（capabilities.tools.listChanged 声明了但从不真发是历史 bug）。
    if (toolListSignature() !== beforeSig) {
        notifyToolsChanged();
    }
}

/** 工具列表签名：editors 的 shortName:工具数 排序拼接。变化即代表暴露给客户端的聚合工具列表变了。 */
...
/** 标记客户端已完成 MCP initialize 握手——之前发 list_changed 没意义（client 尚未就绪）。 */
var clientInitialized = false;

/** 发 MCP notifications/tools/list_changed，让客户端重新请求 tools/list 拿最新聚合列表。 */
